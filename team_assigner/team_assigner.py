...
class TeamAssigner:
    ...
    def get_player_color(self, frame, bbox):
        ...
        top_half_image = image_crop[0:image_crop.shape[0] // 2, :, :]
        
        # Kmeans clustering to find dominant colors
        kmeans = self.get_clustering_model(top_half_image)
        
        # Get the cluster labels for the corners to identify non-player color
        labels = kmeans.labels_
        
        # Reshape labels to match the image crop shape
        clustered_image = labels.reshape(top_half_image.shape[0], top_half_image.shape[1])
        
        # Get the player color cluster
    ...
    def assign_team_color(self, frame, player_detections):
        ...
        kmeans = KMeans(n_clusters=2, init='k-means++', n_init=1, random_state=0).fit(player_colors)
